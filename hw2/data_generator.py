...
def generate(number=100, seed=None, plot=True, num_class=3,sigma=1.0):
    ...
    class_number = (number // num_class) #각 클래스에 속한 데이터포인트 수
    ...
    for i in range(num_class):
        ...
        #sigma2로 데이터 퍼짐 정도 조정, transpose하여 2*class_number mat 생성, uniform: noise 역할 
        ...
        if output_value is None:
          ...
          output_value=np.concatenate((labels,output_value),axis=1) #2차원 이상 결합
    ...
    output_value = output_value.reshape((-1,)) #1차원 배열로 변환
