...
def view_as_windows(arr_in, window_shape, step=1):
    ...
    if not torch.is_tensor(arr_in): #arr_in이 Pytorch tensor인지 check
        raise TypeError("`arr_in` must be a pytorch tensor")

    ndim = arr_in.ndim #dimension

    if isinstance(window_shape, numbers.Number): #숫자일 때 ndim 길이의 tuple 생성
    ...
    if ((arr_shape - window_shape) < 0).any(): #.any: 하나라도 그 조건을 만족하면
...
class nn_convolutional_layer:
    ...
    def forward(self, x):
        ...
        windows = view_as_windows(x, (1, in_ch, f_h, f_w))
        windows = windows.reshape(batch_size, out_h, out_w, -1)  # Flatten windows

        # Perform convolution using matrix multiplication
        filters = self.W.reshape(out_ch, -1).T
        out = torch.matmul(windows, filters).permute(0, 3, 1, 2)

        # Add bias and reshape to output size
        out = out + self.b
        return out
...
class nn_max_pooling_layer:
    ...
    def forward(self, x):
        ...
        windows = view_as_windows(
            x, (1, 1, self.pool_size, self.pool_size), 
            step=(1, 1, self.stride, self.stride)
        )
        # Reshape to prepare for pooling
        windows = windows.reshape(batch_size, in_ch, out_h, out_w, -1)
